...
Examples:
    ...
    - If single inference (not ensamble), don't need to pass it

    python -m classification.fishing.evaluate \
...
PATH = "classification/fishing/evals"
...
def evaluate(pred, data, index, cfg, classes=CLASSES):
    ...
    # FIXME: Filter out Pete's missing samples
    if 0:
        df_pete = pd.read_csv(
            "/Users/fspaolo/dev/sentinel-1-ee/data/fishing/"
            "test_v4_drop_na_unknown.csv"
        )
        _, idx, _ = np.intersect1d(
            detect_id, df_pete.detect_id, return_indices=True
        )
        detect_id, label, Y_true, Y_pred = (
            detect_id[idx],
            label[idx],
            Y_true[idx],
            Y_pred[idx],
...
def main(cfg):
    ...
    # index = zarr_open(cfg.index.replace('v2', 'v2b'))  # FIXME
