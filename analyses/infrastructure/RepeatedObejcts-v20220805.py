...
----------------
composite_table as (
   SELECT * FROM `world-fishing-827.proj_sentinel1_v20210924.detect_comp_raw_*`
...
with ThreadPoolExecutor(max_workers=50) as e:
    for index, row in df.iterrows():
        ...
        table_id = f"world-fishing-827.proj_global_sar.detect_comp_repeat_{t}"
...
---------------
  ...
  SELECT
    ...
    avg_length_m
  FROM
    `world-fishing-827.proj_global_sar.detect_comp_repeat_*` ),
  ...
  FROM
    repeats_composits a
  CROSS JOIN
    `world-fishing-827.proj_sentinel1_v20210924.detect_scene_match` b
  WHERE
...
# +
table_id = f"world-fishing-827.proj_global_sar.infrastructure_repeat_cat_6m_v20220805"
