...
def create_extrap(the_date):
    ...
    from `world-fishing-827.proj_sentinel1_v20210924.detect_scene_score`
    ...
    query_to_table(q, f"world-fishing-827.proj_global_sar.extrap_mult${the_date:%Y%m%d}")
...
def create_extrap_single(the_date):
    ...
    query_to_table(q, f"world-fishing-827.proj_global_sar.extrap_mult_single${the_date:%Y%m%d}")
...
#     pyperclip.copy(q)
    query_to_table(
        q, f"world-fishing-827.proj_global_sar.score_mult_{value}${the_date:%Y%m%d}"
    )
...
#     pyperclip.copy(q)
    query_to_table(
        q, f"world-fishing-827.proj_global_sar.score_single${the_date:%Y%m%d}"
    )
...
# query_to_table(
#     q, f"world-fishing-827.proj_global_sar.frac_diff_lookup"
# )
# -
...
old_score as (
  ...
  score as score_ave
FROM 
  `world-fishing-827.proj_sentinel1_v20210924.detect_scene_score` 
...
from
  without_row_numbers '''
    ...
    query_to_table(
        q, f"world-fishing-827.proj_global_sar.score_combined${the_date:%Y%m%d}"
    )
...
#     pyperclip.copy(q)
    ...
    query_to_table(
        q, f"world-fishing-827.{output_table}${the_date:%Y%m%d}"
    )
