...
q_template = '''SELECT st_union_agg(
  st_geogfromtext(
  footprint_wkt)) as unioned FROM `world-fishing-827.proj_sentinel1_v20210924.detect_foot_raw_{suffix}` 
  where safe.st_geogfromtext(
  footprint_wkt) is not null'''


def process_day(date):
    table_id = f'world-fishing-827.scratch_david.s1f_{date:%Y%m%d}'
...
for year in range(2018,2022):
    for month in range(1,13):
        ...
        table_id = f'world-fishing-827.scratch_david.s1m_{YYYYMM}'
...
for year in range(2017,2022):
    ...
    table_id = f'world-fishing-827.scratch_david.s1y_{year}'
    query_to_table(q_grouped_year, table_id)


q = ''' 
select st_union_agg(unioned) as unioned 
    from `world-fishing-827.scratch_david.s1y_*`
    where _table_suffix between '2017' and '2021' 
'''
table_id =  f'world-fishing-827.scratch_david.s1_footprint_2017_2022'
...
'''
table_id =  f'world-fishing-827.scratch_david.s1_footprint_bridges'
query_to_table(q, table_id)
...
table_id =  f'world-fishing-827.scratch_david.area_near_bridges'
query_to_table(q, table_id)
...
table_id =  f'world-fishing-827.scratch_david.s1_footprint_bridges_3500buffer_highway_primary'
query_to_table(q, table_id)
...
sat_positions as ( select
                  ...
                  lon, lat, altitude
                  from
       `world-fishing-827.satellite_positions_v20190208.sentinel_1_positions*`
                  where
...
`world-fishing-827.proj_sentinel1_v20210924.detect_foot_raw_{YYYYMMDD}`
)
),
...
def process_day(d):
    ...
    table_id = f"world-fishing-827.scratch_david.doppler_removed_{d:%Y%m%d}"
    query_to_table(
...
def copydoppler(the_date):
    ...
    query_to_table(q, f"world-fishing-827.proj_global_sar.doppler_road_area${the_date:%Y%m%d}")
...
def make_doppler_detection_table(the_date):
    q = f'''
     ...
     from 
       `world-fishing-827.proj_sentinel1_v20210924.detect_scene_raw_{the_date:%Y%m%d}`
     left join 
       proj_global_sar.doppler_road_area 
     using(scene_id)
     where date(_partitiontime) = "{the_date:%Y-%m-%d}" ''' 
    
    query_to_table(q, f"world-fishing-827.proj_global_sar.doppler_road_area_detections${the_date:%Y%m%d}")
