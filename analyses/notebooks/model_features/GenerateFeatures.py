...
good_segs as (
  select seg_id from `world-fishing-827.gfw_research.pipe_v20201001_segs`
...
from
  `world-fishing-827.gfw_research.pipe_v20201001_5min`
join
  good_vessels
using(ssvid)
join 
...
# +
# query_to_table(q, 'world-fishing-827.proj_global_sar.ais_vessel_density_2017_2021_100th_degree')
# -
...
sum(if(is_cargo_tanker,hours/(111*111/100/100*cos(3.14/180*lat_index/100)),0)) cargo_tanker_100 
 from `world-fishing-827.proj_global_sar.ais_vessel_density_2017_2021_100th_degree`
...
query_to_table(q, 'world-fishing-827.proj_global_sar.ais_vessel_density_2017_2021_multi_res')
...
### $40 query!!!!
# query_to_table(q, 'world-fishing-827.proj_global_sar.ais_vessel_density_2017_2021_100th_degree_v2')
...
sum(if(not is_fishing_vessels and under_50m,hours/(111*111/100/100*cos(3.14/180*lat_index/100)),0)) non_fishing_under50m_100, --under_50m
sum(if(is_cargo_tanker,hours/(111*111/100/100*cos(3.14/180*lat_index/100)),0)) cargo_tanker_100 
 from `world-fishing-827.proj_global_sar.ais_vessel_density_2017_2021_100th_degree_v2`
 group by lat_index, lon_index
),
...
query_to_table(q, 'world-fishing-827.proj_global_sar.ais_vessel_density_2017_2021_multi_res_v2')
# -
...
SELECT 
  ...
  count(*) overpasses
 FROM 
   `world-fishing-827.proj_sentinel1_v20210924.detect_foot_raster_200` 
 WHERE 
...
# expensive!!!
# query_to_table(q, 'world-fishing-827.proj_global_sar.overpasses_200_by_year_filtered_v20220508')
# -
...
raw_detections as (
  select 
    ...
    *,
    ...
    extract(month from _partitiontime) month
  FROM 
    `world-fishing-827.proj_sentinel1_v20210924.detect_scene_match` 
  join
    (select 
      detect_id, avg(presence) presence, 
      avg(length_m) length_m 
...
query_to_table(q, 'world-fishing-827.proj_global_sar.detections_w_overpasses_v20220509')
# -
...
query_to_table(q, 'world-fishing-827.proj_global_sar.vessel_density_statistics_100th_2017_2021')
# +
...
query_to_table(q, 'world-fishing-827.proj_global_sar.vessel_density_statistics_2017_2021')
# -
...
'''
query_to_table(q, 'world-fishing-827.proj_global_sar.bathymetry_port_shore_metrics_100th')
...
from 
  `world-fishing-827.proj_sentinel1_v20210924.detect_foot_raster_10`
),
...
st_buffer(
 ...
 FROM 
   `world-fishing-827.proj_sentinel1_v20210924.detect_foot_raw_*`
 join
   limit_scene_table
  using
   (scene_id)
...
query_to_table(q, 'world-fishing-827.proj_global_sar.bathymetry_port_shore_metrics_100th_clipped')
# -
...
query_to_table(q, 'world-fishing-827.proj_global_sar.slope')
...
query_to_table(q, 'world-fishing-827.proj_global_sar.average_depth_4km_radius')
...
query_to_table(q, 'world-fishing-827.proj_global_sar.average_depth_2km_radius')
# -
...
q = '''with base_table as (
  select * from 
  `world-fishing-827.environmental_data.copernicus_global_analysis_forecast_phy_001_024_hourly_t_u_v_ssh` 
...
query_to_table(q, 
    'world-fishing-827.proj_global_sar.temp_currents_12thdegree_2017_2019')
...
query_to_table(q, 
    'world-fishing-827.proj_global_sar.temperature_currents_100')
# -
...
query_to_table(q, 
    'world-fishing-827.proj_global_sar.chlorophyll_100')
# +
q = """
...
query_to_table(q, "world-fishing-827.proj_global_sar.detections_w_features_v20220509")
# -
...
query_to_table(q, 'world-fishing-827.proj_global_sar.detections_w_features_v20220720')
...
select 
  ...
  on_fishing_list_nn
from 
  `world-fishing-827.gfw_research.vi_ssvid_v20220701` 
),
...
query_to_table(q, 'world-fishing-827.proj_global_sar.detections_w_features_v20220812')
# -
