...
def window_range(start, months=12, window=6):
    ...
    List of montly time intervals: [(t1, t2), (t3, t4)..].
    It assumes one month = 30 days (half month = 15 days).
    ...
    halfw = timedelta(days=int(window * 15))
    dates = [date(Y, k, D) for k in range(M, months + 1)]
...
def tile_range(xmin, xmax, ymin, ymax, dx, dy):
    ...
    BBOX width and height must be multiple of tile dx and dy.

    xmin, xmax, ymin, ymax : float
        region bbox to be tiled
    dx, dy : float
        tile width and height

    NOTE: Output must be lists (not ndarrays)!
