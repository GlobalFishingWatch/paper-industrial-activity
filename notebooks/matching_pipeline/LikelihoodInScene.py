...
def create_extrap_interesting(the_date):
    ...
    '''
    query_to_table(q, f"world-fishing-827.proj_global_sar.extrap_scene_edge${the_date:%Y%m%d}")
...
#     pyperclip.copy(q)
    query_to_table(
        q, f"world-fishing-827.proj_global_sar.mult_inside_{value}${the_date:%Y%m%d}"
    )
...
#     pyperclip.copy(q)
    ...
    query_to_table(
        q, f"world-fishing-827.proj_global_sar.likelihood_inside${the_date:%Y%m%d}"
    )
...
extrap_raw as (
  select * from 
     (select *, row_number() over (partition by ssvid, scene_id order by rand()) row
       from 
      `world-fishing-827.proj_sentinel1_v20210924.detect_foot_ext_ais`
      WHERE DATE(_PARTITIONTIME) = the_date()
      )
   where row = 1
),
...
using(scene_id, ssvid)'''
    ...
    query_to_table(
        q, f"world-fishing-827.proj_global_sar.expected_recall${the_date:%Y%m%d}"
    )
...
from 
  (select 
    ...
    from `world-fishing-827.proj_sentinel1_v20210924.detect_foot_raw_{the_date:%Y%m%d}` )
...
using(scene_id)'''
    query_to_table(
    q, f"world-fishing-827.proj_global_sar.likely_loc_in_scene${the_date:%Y%m%d}"
)
